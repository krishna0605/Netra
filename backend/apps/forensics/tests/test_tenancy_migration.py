...
class TenancySchemaTests(TestCase):
    ...
    def test_schema_has_49_domain_and_10_framework_tables(self):
        domain_tables = {model._meta.db_table for model in apps.get_models() if model._meta.app_label == "forensics"}
        self.assertEqual(len(domain_tables), 49)
        self.assertEqual(MigrationRecorder.Migration.objects.filter(app="forensics").count(), 22)
        ...
        self.assertTrue(MigrationRecorder.Migration.objects.filter(app="forensics", name="0022_seed_permission_catalogue").exists())
...
class MigrationHarnessRestoresLatestSchemaTests(MigrationHarnessMixin, TransactionTestCase):
    ...
    def test_latest_migration_tracks_the_graph_leaf(self):
        self.assertEqual(latest_migration(), [("forensics", "0022_seed_permission_catalogue")])
