...
from common.credential_mail import send_credential
from common.credential_vault import open_envelope, seal
...
class AccountChange:
    profile: UserProfile
    password: str = ""
    # How the account was handed over, and whether the message actually left.
    # The console reads both: an operator who believes an email was sent when it
    # was not will wait for an officer who never receives anything.
    delivery: str = "invite"
...
def _hold_credential(profile: UserProfile, password: str) -> None:
    """Store the issued password where an administrator can read it back.

    Refuses rather than degrades. If the deployment cannot encrypt, the only
    alternative is a password at rest in the clear, and an operator who asked to
    hold credentials has not asked for that.
    """
    envelope = seal(
        password=password,
    ...
    UserProfile.objects.filter(pk=profile.pk).update(
        held_credential=envelope,
        held_credential_set_at=timezone.now(),
    )
    profile.held_credential = envelope


def reveal_held_credential(*, actor: Actor, organization: Organization, user_id: int, reason: str) -> str:
    """Read back a held password, and record that somebody did.

    Guarded like a destructive operation because it is one in effect: the reader
    can afterwards sign in as that officer. Step-up, a written reason, and an
    entry in the audit chain, so the ledger at least says who looked and why.
    """
    ...
    if not profile.held_credential:
        raise AdministrationProblem(
            "no_held_credential",
            "No password is held for this account. Set one to hand it over.",
            404,
        )

    password = open_envelope(
        profile.held_credential,
        profile_id=profile.pk,
        organization_id=profile.organization_id,
    )
    ...
    record_admin_event(
        ...
        actor=actor,
        action="credential.password_revealed",
        target_type="User",
        target_id=profile.user.get_username(),
        after={"revealedTo": actor.user},
        reason=reason,
    )
    return password
...
def provision_account(
    *,
    ...
    """Create an account and hand it over one of two ways.

    `delivery` is "invite" or "password". An invitation sends the officer a
    magic link and no credential; a password is generated here, shown once to
    the administrator, and optionally mailed. The choice belongs to the operator
    because it depends on something Netra cannot see: whether the officer can
    actually receive mail from this deployment. An invitation to an address that
    never receives it leaves an account nobody can enter, which is why the
    password path exists at all.

    Netra still generates the credential. An administrator who chose it would
    know a password the officer signs in with, and the access log could no
    longer separate the two of them, which is most of what the log is for.
    """
    require_recent_factor(actor)
    ...
    delivery = (delivery or "").strip().lower()
    if not delivery:
        # No explicit choice: keep whatever the deployment was already doing.
        delivery = "invite" if settings.NETRA_AUTH_INVITATIONS_ENABLED else "password"
    if delivery not in {"invite", "password"}:
        raise AdministrationProblem("invalid_delivery", "Choose an invitation or a password.", 400)
    if delivery == "invite" and not settings.NETRA_AUTH_INVITATIONS_ENABLED:
        raise AdministrationProblem(
            "invitations_disabled",
            "Invitations are switched off for this deployment. Issue a password instead.",
            409,
        )

    # A password the officer is never asked to change is one two people know for
    # as long as the account exists. Allowed, because a unit that issues every
    # credential centrally has a real reason to want it, but only where the
    # deployment has said so.
    must_change = not settings.NETRA_ADMIN_HELD_PASSWORDS_ENABLED
    ...
    try:
        if delivery == "invite":
            ...
            # Generated unless the administrator typed one. Either way the
            # strength floor is applied here rather than in the browser only.
    ...
    with transaction.atomic():
        ...
        profile = UserProfile.objects.create(
            ...
            must_change_password=bool(password) and must_change,
            mfa_reset_required=True,
        )
        if password and not must_change:
            # Only where the deployment holds credentials. A password the
            # officer is required to change would be stale the moment they do,
            # and storing one nobody can rely on is worse than storing none.
            _hold_credential(profile, password)
        record_admin_event(
            organization=organization,
            actor=actor,
            action="user.created",
    ...
    # Outside the transaction on purpose. The account exists and is recorded;
    # a mail host that is slow, refusing, or unconfigured must not roll that
    # back, and the credential is on screen regardless of what happens here.
    sent, failure = False, ""
    if password:
        ...
        sent, failure = result.sent, result.reason
...
def replace_password(
    *, actor: Actor, organization: Organization, user_id: int, reason: str, password: str | None = None
    ...
    password = resolve_password(password)
    hold = settings.NETRA_ADMIN_HELD_PASSWORDS_ENABLED
    try:
    ...
    with transaction.atomic():
        UserProfile.objects.filter(pk=profile.pk).update(must_change_password=not hold)
        if hold:
            _hold_credential(profile, password)
        else:
            # Not held: clear any earlier copy rather than leaving one that no
            # longer opens the account.
            UserProfile.objects.filter(pk=profile.pk).update(
                held_credential={}, held_credential_set_at=None
            )
...
def account_payload(profile: UserProfile) -> dict[str, Any]:
    return {
        ...
        # Whether a password is held, never the password. The console needs to
        # know if there is anything to reveal; it does not need the value until
        # somebody asks for it through the endpoint that records the asking.
