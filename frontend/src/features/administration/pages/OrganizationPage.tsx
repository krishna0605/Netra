...
export function OrganizationPage() {
  ...
  useEffect(() => {
    const reseed = window.setTimeout(() => setForm({
      name: organization.name,
      maxQueuedAnalyses: organization.maxQueuedAnalyses,
      accessLogRetentionDays: organization.accessLogRetentionDays,
    }), 0);
    return () => window.clearTimeout(reseed);
  }, [organization]);
            ...
            {owner ? (
              <div className="flex flex-wrap items-center gap-3 px-5 py-4">
                ...
                <div className="min-w-0 flex-1">
                  <Link to={`/administration/users/${owner.id}`} className="text-[15px] text-cream-bright hover:text-signal">
                    {owner.name}
                  </Link>
