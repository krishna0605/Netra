...
export function PasswordDialog({
  ...
  const canSubmit = reasonValid && code.trim().length === 6 && !busy;
  ...
  async function submit() {
    ...
    try {
      ...
      const applied = await setPassword({ userId: user.id, reason: reason.trim() });
            ...
            <>
              <div className="flex flex-col gap-4 overflow-y-auto px-6 py-5">
                <p className="rounded-panel border border-[color:var(--color-control-edge)] bg-cream-primary/4 px-4 py-3 text-[13px] leading-relaxed text-sand-muted/70">
                  Netra generates the temporary credential on the server. You cannot view the current password or choose a permanent replacement for this user.
                </p>
